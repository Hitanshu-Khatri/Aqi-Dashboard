...
export const ShareReportButton: React.FC<ShareReportButtonProps> = ({ 
  ...
  const exportAsPNG = async () => {
    try {
      ...
      ctx.fillText('🌍 Air Quality Report', canvas.width / 2, 50);
      
      // Location and timestamp
      ctx.font = '16px Inter, sans-serif';
      ctx.fillStyle = '#94a3b8';
      ctx.fillText(`📍 ${location}`, canvas.width / 2, 80);
      ctx.fillText(`🕒 Generated: ${new Date(sensorData.timestamp).toLocaleString()}`, canvas.width / 2, 105);
      ...
      // Metrics grid
      const metrics = [
        { label: '🌡️ Temperature', value: sensorData.temperature, unit: '°C', x: 200, y: 320 },
        { label: '💧 Humidity', value: sensorData.humidity, unit: '%', x: 600, y: 320 },
        { label: '☁️ PM2.5', value: sensorData.pm2_5, unit: 'μg/m³', x: 200, y: 420 },
        { label: '🌫️ pm10', value: sensorData.pm10, unit: 'μg/m³', x: 600, y: 420 }
      ];
...
📍 Location: ${location}
🕒 Generated: ${new Date(sensorData.timestamp).toLocaleString()}
...
Health Implications:
${aqiInfo.level === 'Good' ? '✅ Air quality is satisfactory, and air pollution poses little or no risk.' :
  aqiInfo.level === 'Moderate' ? '⚠️ Air quality is acceptable. However, there may be a risk for some people.' :
  aqiInfo.level.includes('Unhealthy') ? '❌ Everyone may begin to experience health effects.' :
...
• Consider indoor air purification if AQI > 100
